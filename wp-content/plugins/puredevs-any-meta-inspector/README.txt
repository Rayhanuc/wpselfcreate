...
Tags: metabox, metadata, user metadata, post metadata, category metadata, comment metadata
...
When activated, PureDevs Any Meta Inspector shows all the meta keys and their unserialized values in a metabox for posts( including CPT ), pages, terms( any taxonomy ), comments and users. This metabox will only be displayed for either administrator users or users with `manage_options` capability.
...
3. Inspect custom post type(CTP) metadata
